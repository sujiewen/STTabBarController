...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        ...
        UITableViewController *viewController0 = [[UITableViewController alloc] init];
        UINavigationController *navigationController0 = [[UINavigationController alloc] initWithRootViewController:viewController0];
...
- (void)viewDidLoad {
    ...
    self.tabBar.hitTestBlock = ^(CGPoint point, UIEvent *event, BOOL *returnSuper) {
        /// 按钮的点击区域距离圆心的距离
        CGFloat distance2 = ABS((point.x - publishCenterPoint.x) * (point.x - publishCenterPoint.x) + (point.y - publishCenterPoint.y) * (point.y - publishCenterPoint.y));
        
        CGFloat distance = sqrtf(distance2);
...
//    SKTestViewController *viewVC = [[SKTestViewController alloc] init];
//    [self presentViewController:viewVC
//                            animated:YES
//                          completion:^{
//     
//                          }];
